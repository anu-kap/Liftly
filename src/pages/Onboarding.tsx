  ...
  return (
    <div className="flex min-h-dvh flex-col px-6 py-10">
      <div className="mb-8 flex gap-1.5">
        {[0, 1, 2, 3].map(i => (
          <div key={i} className={`h-1 flex-1 rounded-full transition-colors ${i <= step ? 'bg-violet-500' : 'bg-white/10'}`} />
        ))}
      </div>

      {step === 0 && (
        <div className="animate-pop flex flex-1 flex-col">
          <h1 className="text-4xl font-bold leading-tight">Welcome to <span className="grad-text">Liftly</span></h1>
          ...
          <div className="mt-8 space-y-3">
            {([
              ...
              <button
                ...
                className={`card w-full p-4 text-left transition-all ${focus === key ? 'border-violet-500/60 bg-violet-500/10' : ''}`}
              >
      ...
      {step === 1 && (
        <div className="animate-pop flex flex-1 flex-col">
          ...
          <p className="mt-8 mb-2 text-sm font-medium text-zinc-300">Experience level</p>
          ...
          <p className="mt-6 mb-2 text-sm font-medium text-zinc-300">Workout days per week</p>
          <div className="flex gap-2">
            {[2, 3, 4, 5, 6].map(n => (
          ...
          <p className="mt-6 mb-2 text-sm font-medium text-zinc-300">Units</p>
          <div className="flex gap-2">
        ...
        <div className="animate-pop flex flex-1 flex-col">
          ...
          <div className="mt-6 space-y-3 overflow-y-auto pb-4">
            {GOAL_CANDIDATES.map(id => {
              ...
              return (
                <div key={id} className={`card p-4 ${goal ? 'border-violet-500/60' : ''}`}>
                  <button className="flex w-full items-center justify-between" onClick={() => toggleGoal(id)}>
                    <span className="font-medium">{ex.name}</span>
                    <span className={`flex h-6 w-6 items-center justify-center rounded-full border text-xs ${goal ? 'border-violet-500 bg-violet-500 text-white' : 'border-white/20 text-transparent'}`}>✓</span>
                  </button>
                  {goal && (
      ...
      {step === 3 && (
        <div className="animate-pop flex flex-1 flex-col">
          ...
          <p className="mt-2 text-zinc-400">
            ...
            During each workout you'll get exact set × rep × weight targets that progress you toward your goals.
          </p>
          <div className="card mt-6 space-y-2 p-4 text-sm text-zinc-300">
            <p>✦ {days} workouts per week, tailored to {focus === 'strength' ? 'strength' : focus === 'hypertrophy' ? 'muscle growth' : 'overall fitness'}</p>
            <p>✦ {goals.length > 0 ? `${goals.length} lift goal${goals.length > 1 ? 's' : ''} with pace tracking` : 'Add lift goals anytime in Profile'}</p>
            <p>✦ Progressive overload coaching in every session</p>
            <p>✦ Sign in with Google later to back up your data</p>
          </div>
          <div className="mt-auto flex gap-2">
